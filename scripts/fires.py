...
file_path = '6.- Ocurrencia Nacional de Incendios Forestales según Mes, 1985 - 2024_octubre.xls'

dfdictionary = pd.read_excel(file_path, sheet_name=None)
for i in dfdictionary:
    dfdictionary[i] = clean_conaf_exc(dfdictionary[i], 'MES')      
with pd.ExcelWriter("CantidadIncendiosClean.xlsx") as writer:
    for i in range(2023,2002, -1):
        df = dfdictionary[str(i)]
        df.to_excel(writer, sheet_name=str(i), index= 'MES')
